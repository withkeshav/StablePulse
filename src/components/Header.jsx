...
export default function Header({ loading, apiStatus, onOpenSettings, buildVersion, theme, setTheme, alertCount, onJumpToAlerts }) {
  ...
  return (
    <header id="header">
      <div class="logo">StableScope {buildVersion ? <span class="build-version">{buildVersion}</span> : null}</div>
      <div class="status-bar">
        <div id="status-dot" class={statusClass}></div>
        <span id="status-text">{loading ? 'Syncing...' : anyFailed ? 'Partial' : 'Live'}</span>
      </div>
      ...
      <div class="hdr-actions">
        <button type="button" id="settings-btn" class="hdr-btn" onClick={onOpenSettings} title="Settings">
          ⚙
        </button>
      </div>
    </header>
